...
class UserRepositoryImpl extends UserRepository {
  ...
  val u = UserRecord.syntax

  def findAll(): List[User] = {
    val records = DB.readOnly { implicit session =>
      withSQL {
        select(u.result.*).from(UserRecord as u)
  ...
  def create(user: User) = {
    ...
    DB.localTx { implicit session =>
      withSQL {
        insert.into(UserRecord).namedValues(record.columnsAndValues: _*)
      }.update.apply()
    }
  }
...
object UserRepositoryImpl {
  ...
  case class UserRecord(id: String, name: String) {
    ...
    def columnsAndValues = List(
      UserRecord.syntax.id   -> this.id,
      UserRecord.syntax.name -> this.name
    )
  }
  ...
  object UserRecord extends SQLSyntaxSupport[UserRecord] {
    ...
    def fromEntity(entity: User): UserRecord = {
      UserRecord(entity.id.uuid.toString, entity.name)
    }
